...
- `NoNoncense` raises explicit `NoNoncense.Errors.DisabledError` and `NoNoncense.Errors.UninitializedError` when appropriate, instead of relying on the `:persistent_term.get/1` argument error.

## [1.3.1]

- Use bitwise operations to process atomics
- Fix sortable nonce initialization (this did not cause problems in practice but was technically incorrect)
